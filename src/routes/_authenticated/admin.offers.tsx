...
function toDraft(row: Record<string, unknown>): OfferDraft {
  ...
  return {
    ...
    parent_offer_id: s("parent_offer_id"),
    input_currency: s("input_currency") || "USD",
    badge_id: s("badge_id"),
    offer_type: s("offer_type") || "tourism_package",
    customer_price_usd: String(
      row["input_price"] ?? row["customer_price_usd"] ?? row["base_price_usd"] ?? "",
    ),
    agency_price_usd: String(row["input_agency_price"] ?? row["agency_price_usd"] ?? ""),
    original_price_usd: String(row["input_original_price"] ?? row["original_price_usd"] ?? ""),
